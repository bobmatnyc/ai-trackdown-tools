...
describe('Epic Command Tests', () => {
  ...
  describe('Epic Create Command', () => {
    it('should create a new epic with required fields', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'epic', 'create', 'Test Epic', '--description', 'A test epic', '--assignee', 'test-user'], { from: 'user' });
    ...
    it('should handle missing description with interactive prompt', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'epic', 'create', 'Interactive Epic'], { from: 'user' });
    ...
    it('should handle epic creation with flexible options', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'epic', 'create', 'Flexible Epic',
          '--description', 'Epic with all options',
          '--priority', 'high',
          '--assignee', 'test-user',
  ...
  describe('Epic List Command', () => {
    ...
    it('should filter epics by status', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'epic', 'list', '--status', 'active'], { from: 'user' });
    ...
    it('should filter epics by assignee', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'epic', 'list', '--assignee', 'test-user'], { from: 'user' });
  ...
  describe('Epic Show Command', () => {
    ...
    it('should handle non-existent epic', async () => {
      ...
      try {
        ...
        expect(consoleMock.errors.some(error => error.includes('not found') || error.includes('EP-9999'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
  ...
  describe('Epic Update Command', () => {
    ...
    it('should handle invalid epic ID', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'epic', 'update', 'INVALID-ID',
          '--status', 'completed'
        ], { from: 'user' });
  ...
  describe('Epic Complete Command', () => {
    it('should mark epic as completed', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'epic', 'complete', 'EP-0001',
          '--actual-tokens', '1500',
          '--notes', 'Epic completed successfully'
        ], { from: 'user' });
  ...
  describe('Error Handling', () => {
    it('should handle missing tasks directory', async () => {
      ...
      try {
        ...
        expect(consoleMock.errors.some(error => error.includes('not found') || error.includes('No epics found'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
...
`;
      ...
      const epicPath = path.join(testContext.tempDir, 'tasks', 'epics', 'EP-0002-malformed-epic.md');
    ...
    it('should handle file system permission errors', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'epic', 'create', 'Permission Test'], { from: 'user' });
  ...
  describe('Cross-project Epic References', () => {
    ...
    it('should handle invalid project directory', async () => {
      ...
      try {
        ...
        expect(consoleMock.errors.some(error => error.includes('not found') || error.includes('directory'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });

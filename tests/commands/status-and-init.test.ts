...
`;
      ...
      fs.writeFileSync(path.join(testContext.tempDir, 'tasks', 'epics', 'EP-0002-completed-epic.md'), completedEpicContent);
...
`;
      ...
      fs.writeFileSync(path.join(testContext.tempDir, 'tasks', 'issues', 'ISS-0002-in-progress-issue.md'), inProgressIssueContent);
    });
    ...
    it('should display basic project status', async () => {
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('Project Status') || log.includes('status'))).toBe(true);
        expect(consoleMock.logs.some(log => log.includes('Epic') || log.includes('Issue'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should display detailed status with --verbose flag', async () => {
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('Test Epic') || log.includes('Test Issue'))).toBe(true);
        expect(consoleMock.logs.some(log => log.includes('active') || log.includes('completed'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should display full status with all items', async () => {
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('Epic') || log.includes('Issue'))).toBe(true);
        expect(consoleMock.logs.some(log => log.includes('Task') || log.includes('PR'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should show progress statistics', async () => {
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('%') || log.includes('progress') || log.includes('completion'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should show token statistics', async () => {
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('token') || log.includes('estimated') || log.includes('actual'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should filter status by priority', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'status', '--priority', 'high'], { from: 'user' });
    ...
    it('should filter status by assignee', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'status', '--assignee', 'test-user'], { from: 'user' });
    ...
    it('should show current sprint status', async () => {
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('sprint') || log.includes('active') || log.includes('progress'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should handle project with no tasks', async () => {
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('No items') || log.includes('empty') || log.includes('0'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should handle status with project directory option', async () => {
      ...
      try {
        await program.parseAsync([
          'node', 'test', 'status',
          '--project-dir', externalProjectDir
        ], { from: 'user' });
        
        // Should show status for external project
        expect(consoleMock.logs.some(log => log.includes('Test Epic') || log.includes('status'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
  ...
  describe('Init Command', () => {
    ...
    it('should initialize with custom templates', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'init', 'custom-project', '--with-templates'], { from: 'user' });
    ...
    it('should initialize with GitHub integration', async () => {
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('GitHub') || log.includes('integration'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should handle interactive initialization', async () => {
      ...
      try {
        ...
        TestAssertions.assertDirectoryExists(projectDir);
        TestAssertions.assertFileExists(path.join(projectDir, 'README.md'));
        TestAssertions.assertFileContains(path.join(projectDir, 'README.md'), 'Interactive project description');
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should handle existing project directory', async () => {
      ...
      try {
        ...
        expect(consoleMock.errors.some(error => error.includes('exists') || error.includes('existing')) ||
               consoleMock.logs.some(log => log.includes('initialized') || log.includes('created'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
    ...
    it('should handle force initialization', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'init', 'force-project', '--force'], { from: 'user' });
    ...
    it('should create sample items when requested', async () => {
      ...
      try {
        await program.parseAsync(['node', 'test', 'init', 'sample-project', '--with-samples'], { from: 'user' });
    ...
    it('should handle initialization errors', async () => {
      ...
      try {
        ...
        expect(consoleMock.errors.some(error => error.includes('permission') || error.includes('EACCES'))).toBe(true);
      } finally {
        consoleMock.restore();
    ...
    it('should validate project name', async () => {
      ...
      try {
        ...
        expect(consoleMock.errors.some(error => error.includes('invalid') || error.includes('name')) ||
               consoleMock.logs.some(log => log.includes('initialized'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
  ...
  describe('Status Error Handling', () => {
    it('should handle corrupted task files', async () => {
      ...
      fs.writeFileSync(path.join(testContext.tempDir, 'tasks', 'epics', 'corrupted.md'), 'invalid content');
      ...
      try {
        ...
        expect(consoleMock.logs.some(log => log.includes('status') || log.includes('epic') || log.includes('issue')) ||
               consoleMock.errors.some(error => error.includes('parse') || error.includes('corrupted'))).toBe(true);
      } finally {
        consoleMock.restore();
      }
    });
...
# Epic ${i}
`;
        fs.writeFileSync(path.join(epicsDir, `EP-${String(i).padStart(4, '0')}-epic-${i}.md`), epicContent);
...
# Issue ${i}
`;
        fs.writeFileSync(path.join(issuesDir, `ISS-${String(i).padStart(4, '0')}-issue-${i}.md`), issueContent);
      }

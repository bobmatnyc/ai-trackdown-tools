...
describe('Project Switching Integration Tests', () => {
  ...
  describe('multi-project workflows', () => {
    ...
    async function createProjectWithGitRepo(projectName: string, files: Record<string, string>) {
      ...
      try {
        ...
        execSync(`git remote add origin https://github.com/test/${projectName}.git`, { stdio: 'ignore' });
      } finally {
        process.chdir(originalCwd);
      }
    }
    ...
    it('should switch between projects with different contexts', async () => {
      ...
      expect(fs.existsSync(path.join(frontendPaths.issuesDir, 'ISS-001-frontend-issue.md'))).toBe(true);
      expect(fs.existsSync(path.join(backendPaths.issuesDir, 'ISS-001-backend-issue.md'))).toBe(true);
      
      // Verify cross-project isolation
      expect(fs.existsSync(path.join(frontendPaths.issuesDir, 'ISS-001-backend-issue.md'))).toBe(false);
      expect(fs.existsSync(path.join(backendPaths.issuesDir, 'ISS-001-frontend-issue.md'))).toBe(false);
    });
  ...
  describe('project creation and initialization workflows', () => {
    ...
    it('should create new project and initialize with git metadata', async () => {
      ...
      execSync('git add .', { stdio: 'ignore' });
      execSync('git commit -m "Initial commit"', { stdio: 'ignore' });
      execSync('git remote add origin https://github.com/test/new-project.git', { stdio: 'ignore' });
      ...
      for (let i = 0; i < 3; i++) {
        for (const projectName of projectNames) {
          ...
          for (const otherProject of otherProjects) {
            expect(fs.existsSync(path.join(paths.issuesDir, `${otherProject}-issue.md`))).toBe(false);
          }
        }
      }
    });
  ...
  describe('environment and configuration handling', () => {
    ...
    it('should handle configuration inheritance in multi-project mode', async () => {
      ...
      fs.writeFileSync(rootConfigPath, JSON.stringify({

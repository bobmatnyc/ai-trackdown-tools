...
describe('CLI Multi-Project E2E Tests', () => {
  ...
  describe('single-project mode CLI commands', () => {
    ...
    it('should create epic in single-project mode', async () => {
      const result = runCLI(['epic', 'create', '--title', 'Test Epic', '--description', 'Epic description']);
    ...
    it('should create issue in single-project mode', async () => {
      ...
      const epicResult = runCLI(['epic', 'create', '--title', 'Parent Epic', '--description', 'Epic for issues']);
      ...
      // Create issue
      const issueResult = runCLI(['issue', 'create', '--title', 'Test Issue', '--description', 'Issue description', '--epic', epicId]);
  ...
  describe('multi-project mode CLI commands', () => {
    ...
    it('should create epic in specific project', async () => {
      const result = runCLI(['epic', 'create', '--title', 'Frontend Epic', '--description', 'Epic for frontend', '--project', 'frontend']);
    ...
    it('should create issue in specific project', async () => {
      ...
      const epicResult = runCLI(['epic', 'create', '--title', 'Backend Epic', '--description', 'Epic for backend', '--project', 'backend']);
      expect(epicResult.exitCode).toBe(0);
      ...
      const issueResult = runCLI(['issue', 'create', '--title', 'Backend Issue', '--description', 'Issue for backend', '--epic', epicId, '--project', 'backend']);
    ...
    it('should list epics from specific project', async () => {
      ...
      runCLI(['epic', 'create', '--title', 'Frontend Epic 1', '--description', 'FE epic 1', '--project', 'frontend']);
      runCLI(['epic', 'create', '--title', 'Frontend Epic 2', '--description', 'FE epic 2', '--project', 'frontend']);
      runCLI(['epic', 'create', '--title', 'Backend Epic 1', '--description', 'BE epic 1', '--project', 'backend']);
    ...
    it('should show status for specific project', async () => {
      ...
      runCLI(['epic', 'create', '--title', 'Mobile Epic', '--description', 'Mobile epic', '--project', 'mobile']);
    ...
    it('should switch projects', async () => {
      ...
      runCLI(['epic', 'create', '--title', 'Frontend Epic', '--description', 'FE epic', '--project', 'frontend']);
      
      // Switch to backend and create epic
      const switchResult = runCLI(['project', 'switch', 'backend']);
      expect(switchResult.exitCode).toBe(0);
      
      const backendEpicResult = runCLI(['epic', 'create', '--title', 'Backend Epic', '--description', 'BE epic']);
    ...
    it('should handle project creation', async () => {
      const result = runCLI(['project', 'create', 'new-project', '--name', 'New Project', '--description', 'A new project']);
  ...
  describe('CLI error handling', () => {
    it('should handle missing project in multi-project mode', async () => {
      ...
      const result = runCLI(['epic', 'create', '--title', 'Test Epic', '--description', 'Test', '--project', 'nonexistent']);
  ...
  describe('CLI with git metadata integration', () => {
    beforeEach(() => {
      ...
      fs.writeFileSync(path.join(testRootPath, 'package.json'), JSON.stringify({
        name: 'test-project',
      ...
      execSync('git remote add origin https://github.com/test/test-project.git', { stdio: 'ignore', cwd: testRootPath });
    });
  ...
  describe('CLI performance and concurrency', () => {
    it('should handle concurrent operations', async () => {
      ...
      const operations = [
        runCLIAsync(['epic', 'create', '--title', 'Epic 1', '--description', 'First epic', '--project', 'project1']),
        runCLIAsync(['epic', 'create', '--title', 'Epic 2', '--description', 'Second epic', '--project', 'project2']),

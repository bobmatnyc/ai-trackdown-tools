...
describe('GitMetadataExtractor - Multi-Project Support', () => {
  ...
  describe('multi-project git metadata extraction', () => {
    ...
    it('should handle multiple projects with different tech stacks', async () => {
      ...
      fs.writeFileSync(path.join(reactProjectDir, 'package.json'), JSON.stringify({
        name: 'react-app',
        dependencies: { 'react': '^18.0.0', 'next': '^13.0.0' }
      }));
      fs.mkdirSync(path.join(reactProjectDir, 'pages'), { recursive: true });
      fs.writeFileSync(path.join(reactProjectDir, 'pages/index.tsx'), 'export default function Home() {}');
      execSync('git add .', { stdio: 'ignore' });
      execSync('git commit -m "Initial commit"', { stdio: 'ignore' });
      ...
      fs.writeFileSync(path.join(pythonProjectDir, 'requirements.txt'), 'Django==4.2.0\ndjango-rest-framework==3.14.0');
      ...
      fs.writeFileSync(path.join(pythonProjectDir, 'api/views.py'), 'from django.http import JsonResponse');
      execSync('git add .', { stdio: 'ignore' });
      execSync('git commit -m "Initial commit"', { stdio: 'ignore' });
  ...
  describe('framework detection edge cases', () => {
    it('should handle monorepo with multiple frameworks', async () => {
      ...
      fs.writeFileSync(path.join(packagesDir, 'frontend', 'package.json'), JSON.stringify({
        name: 'frontend',
        dependencies: { 'next': '^13.0.0' }
      }));
      
      fs.mkdirSync(path.join(packagesDir, 'backend'));
      fs.writeFileSync(path.join(packagesDir, 'backend', 'package.json'), JSON.stringify({
        name: 'backend',
  ...
  describe('project metadata population for multi-project scenarios', () => {
    it('should populate project frontmatter with multi-project context', async () => {
      ...
      execSync('git remote add origin https://github.com/user/test-project.git', { stdio: 'ignore' });
      execSync('git add .', { stdio: 'ignore' });
      execSync('git commit -m "Initial commit"', { stdio: 'ignore' });

...
describe('End-to-End Workflow Integration Tests', () => {
  ...
  describe('Complete Project Lifecycle', () => {
    it('should handle complete project setup to completion workflow', async () => {
      ...
      TestAssertions.assertFileContains(path.join(epicsDir, epicFile), 'User Authentication System');
      ...
      const issueData = [
        { title: 'Implement Login Form', description: 'Create responsive login form with validation' },
      ...
      for (let i = 0; i < issueIds.length; i++) {
        ...
        for (const task of taskData) {
          result = await runCLICommand([
            ...
            '--description', `Implementation task for ${task.title}`,
            '--issue', issueId,
      ...
      result = await runCLICommand([
        'issue', 'complete', issueIds[0],
        '--actual-tokens', '600',
        '--notes', 'All login functionality implemented and tested'
      ]);
      expect(result.success).toBe(true);
      
      // Step 9: Create PRs for completed work
      result = await runCLICommand([
        'pr', 'create', 'Implement User Login System',
      ...
      result = await runCLICommand([
        'epic', 'complete', epicId!,
        '--actual-tokens', '1800',
        '--notes', 'User authentication system fully implemented and deployed'
      ]);
      expect(result.success).toBe(true);
  ...
  describe('Multi-Project Workflow', () => {
    it('should handle multi-project development workflow', async () => {
      ...
      let result = await runCLICommand([
        'epic', 'create', 'Frontend UI Components',
      ...
      result = await runCLICommand([
        'epic', 'create', 'API Development',
        '--description', 'RESTful API endpoints',
        '--priority', 'high',
        '--project-dir', projectPaths[1]
      ]);
      expect(result.success).toBe(true);
  ...
  describe('Error Recovery Workflow', () => {
    it('should handle workflow with various error conditions', async () => {
      ...
      let result = await runCLICommand([
        'epic', 'create', 'Error Recovery Test',
      ...
      result = await runCLICommand([
        ...
        '--description', 'Issue with non-existent epic',
        '--epic', 'EP-9999'
      ]);
  ...
  describe('GitHub Integration Workflow', () => {
    it('should handle GitHub integration workflow', async () => {
      ...
      let result = await runCLICommand([
        'epic', 'create', 'GitHub Integration',
        '--description', 'Testing GitHub integration features'
      ]);
      expect(result.success).toBe(true);
      
      result = await runCLICommand([
        'issue', 'create', 'GitHub Sync Issue',
        '--description', 'Issue for GitHub sync testing',
        '--epic', 'EP-0001'
      ]);
      expect(result.success).toBe(true);
      ...
      result = await runCLICommand([
        'pr', 'create', 'Test GitHub PR',
        '--description', 'PR for testing GitHub integration',
        '--issue', 'ISS-0001',
    ...
    it('should handle large-scale project workflow efficiently', async () => {
      ...
      for (let epicNum = 0; epicNum < 5; epicNum++) {
        for (let issueNum = 0; issueNum < 5; issueNum++) {
          result = await runCLICommand([
            'issue', 'create', `Issue ${epicNum}-${issueNum}`,
            '--description', `Issue ${issueNum} for epic ${epicNum}`,
            '--epic', `EP-${String(epicNum + 1).padStart(4, '0')}`
          ]);
          expect(result.success).toBe(true);
        }
      }
  ...
  describe('Collaborative Workflow', () => {
    it('should handle collaborative development workflow', async () => {
      ...
      let result = await runCLICommand([
        'epic', 'create', 'Team Collaboration Feature',
      ...
      for (let i = 0; i < teamMembers.length; i++) {
        result = await runCLICommand([
          'issue', 'create', `Feature Component ${i + 1}`,
          '--description', `Component implementation by ${teamMembers[i]}`,
          '--epic', 'EP-0001',
          '--assignee', teamMembers[i],
          '--priority', 'medium'
        ]);
        expect(result.success).toBe(true);
      ...
      for (let i = 0; i < issueIds.length; i++) {
        ...
        result = await runCLICommand([
          'task', 'create', `Implementation Task for ${assignee}`,
          '--description', `Implementation work by ${assignee}`,
          '--issue', issueId,
          '--assignee', assignee,
          '--estimated-time', '8h'
        ]);
  ...
  describe('Migration and Data Import Workflow', () => {
    it('should handle data migration workflow', async () => {
      ...
      fs.writeFileSync(path.join(legacyDir, 'old-tasks', 'task1.md'), '# Legacy Task 1\nSome content');
      fs.writeFileSync(path.join(legacyDir, 'old-tasks', 'task2.md'), '# Legacy Task 2\nMore content');
      ...
      result = await runCLICommand([
        'epic', 'create', 'Migrated Epic',
        '--description', 'Epic created after migration'
      ]);
      expect(result.success).toBe(true);

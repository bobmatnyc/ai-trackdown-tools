  ...
  cmd
    ...
    .action(async (options: ListOptions) => {
      try {
        ...
        console.error(Formatter.error(`Failed to list projects: ${error instanceof Error ? error.message : 'Unknown error'}`));
        process.exit(1);
      }
    });
...
async function listProjects(options: ListOptions): Promise<void> {
  ...
  if (mode === 'single') {
    ...
    console.log(Formatter.info('Use "aitrackdown project create" to initialize multi-project mode.'));
    return;
  }
  ...
  if (projects.length === 0) {
    ...
    console.log(Formatter.info('Use "aitrackdown project create <name>" to create your first project.'));
    return;
  }
  ...
  switch (options.format) {
    ...
    default:
      ...
      console.log();
      console.log(Formatter.info('Use "aitrackdown project switch <name>" to switch to a different project.'));
  ...
  if (options.showDetails) {
    ...
    console.log(Formatter.info(`Projects Directory: ${currentContext?.context.projectsDir || 'Not set'}`));

...
import type { ItemStatus, Priority, IssueFrontmatter } from '../../types/ai-trackdown.js';
  ...
  cmd
    ...
    .action(async (issueId: string, options: UpdateOptions) => {
      try {
        ...
        console.error(Formatter.error(`Failed to update issue: ${error instanceof Error ? error.message : 'Unknown error'}`));
        process.exit(1);
      }
    });

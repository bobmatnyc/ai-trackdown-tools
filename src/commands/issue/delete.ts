  ...
  cmd
    ...
    .action(async (issueId: string, options: DeleteOptions) => {
      try {
        ...
        console.error(Formatter.error(`Failed to delete issue: ${error instanceof Error ? error.message : 'Unknown error'}`));
        process.exit(1);
      }
    });
...
async function deleteIssue(issueId: string, options: DeleteOptions): Promise<void> {
  ...
  if (tasks.length > 0 && !options.recursive) {
    ...
    console.log('');
    console.log(Formatter.info('Use --recursive to delete all related tasks, or reassign them first.'));
  ...
  if (related.dependents.length > 0) {
    ...
    if (!options.force) {
      throw new Error('Cannot delete issue with dependents. Use --force to override or remove dependencies first.');
    }
  }
  ...
  if (!options.force) {
    ...
    const answer = await new Promise<string>((resolve) => {
      rl.question(
        Formatter.warning('Are you sure you want to delete this issue and all related tasks? (yes/no): '),
        resolve
      );
    });
  ...
  if (related.dependents.length > 0) {
    console.log(Formatter.warning('Warning: Items that depended on this issue may need to be updated:'));
    for (const dependent of related.dependents) {
      const depId = getItemId(dependent);
      console.log(`  • ${depId}: ${dependent.title}`);
    }

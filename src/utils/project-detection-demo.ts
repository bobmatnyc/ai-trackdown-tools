...
export class ProjectDetectionDemo {
  ...
  async demoSingleProjectMode(): Promise<void> {
    ...
    const directories = ['tasks/epics', 'tasks/issues', 'tasks/tasks', 'tasks/prs', 'tasks/templates'];
    ...
    writeFileSync(join(this.testRoot, '.ai-trackdown/config.yaml'), `
name: demo-project
version: 1.0.0
...
`);
  ...
  async demoMultiProjectMode(): Promise<void> {
    ...
    for (const project of projects) {
      ...
      writeFileSync(join(projectPath, '.ai-trackdown/config.yaml'), `
name: ${project}
version: 1.0.0
...
`);
  ...
  async demoEnvironmentOverrides(): Promise<void> {
    ...
    const directories = ['custom-tasks/epics', 'custom-tasks/issues', 'custom-tasks/tasks', 'custom-tasks/prs', 'custom-tasks/templates'];
    directories.forEach(dir => mkdirSync(join(this.testRoot, dir), { recursive: true }));
...
`);
  ...
  async demoMigrationDetection(): Promise<void> {
    ...
    writeFileSync(join(this.testRoot, 'trackdown/active/EP-001-legacy-epic.md'), '# Legacy Epic\n\nThis is a legacy epic');
    writeFileSync(join(this.testRoot, 'trackdown/active/ISS-001-legacy-issue.md'), '# Legacy Issue\n\nThis is a legacy issue');

    // Create PRJ files (another legacy indicator)
    writeFileSync(join(this.testRoot, 'PRJ-001-legacy-project.md'), '# Legacy Project\n\nThis is a legacy project file');
